...
bool serial::setParameters()
{
 ...
 return !tcsetattr(fileDescriptor, TCSANOW, &terminalAttributes);  // Set options
}
...
int serial::sendString(std::string &message) 
{
 ...
 {
  int n = write(fileDescriptor, ( char*) message.c_str(), strlen(message.c_str()));
...
int serial::readString (std::string &msg)
{
 ...
 while (c!=eol)
 {
  if(read(fileDescriptor, &c, sizeof(char)) > 0 && c!= 0x0A && c!= 0x0D && c!= '>')
  {
   msg += c;
